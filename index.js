...
import ApplicationRedux from './ApplicationRedux';
...
const mathReducer = (state = {result:5, lastValue:[]}, action) => {
	switch(action.type){
		case "ADD":
			//state.result = state.result + action.payload
			state = {...state, result:state.result + action.payload}
			break;
		case "SUB":
			state.result = state.result - action.payload;
			break
...
const store = createStore(combineReducers({math:mathReducer,user:userReducer}), {}, applyMiddleware(loggerMiddleware));
...
ReactDOM.render(
	<Provider store={store} > <ApplicationRedux /> </Provider>
